...
class AudioModel {
    ...
    init(buffer_size:Int) {
        ...
        sinData = Array.init(repeating: 0.0, count: BUFFER_SIZE)
        fftSinData = Array.init(repeating: 0.0, count: BUFFER_SIZE/2)
    }
...
//        }
    ...
    private lazy var sinFftHelper:FFTHelper? = {
        return FFTHelper.init(fftSize: Int32(BUFFER_SIZE))
    }()
    ...
    private func runEveryInterval(){
        if inputBuffer != nil {
            ...
            fftHelper!.performForwardFFT(withData: &sinData,
                                         andCopydBMagnitudeToBuffer: &fftSinData) // fft result is copied into fftData array
    ...
    private func handleSpeakerQueryWithSinusoid(data:Optional<UnsafeMutablePointer<Float>>, numFrames:UInt32, numChannels: UInt32){
        ...
        if let dataPointer = data {
            var mySinData = Array(UnsafeBufferPointer(start: dataPointer, count: BUFFER_SIZE/2))
            sinData = mySinData + mySinData
            // sinData is now of type [Float]
        } else {
